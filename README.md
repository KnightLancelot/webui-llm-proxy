...
| **Kimi 服务端异常重试** | 检测到 Kimi 提示 "服务端异常，请稍后重试" 时，自动等待 60 秒后重发，最多重试 5 次 |
...
Kimi 网页版偶尔会出现 `服务端异常，请稍后重试` 的提示。代理会自动处理：

- 检测到该错误提示或请求未返回任何内容时，等待 **60 秒**
- 使用相同的请求内容自动重新发送
- 最多重试 **5 次**，直到成功或达到最大次数

该逻辑同时适用于 `stream=false`（非流式）和 `stream=true`（流式）请求。
